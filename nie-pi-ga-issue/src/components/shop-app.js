...
class ShopApp extends connect(store)(LitElement) {
  _render({
    ...
    <shop-analytics key="GTM-KLP5G83"></shop-analytics>
  ...
  _didRender(props, changed, oldProps) {
    if ('_page' in changed || '_categoryName' in changed) {
      ...
      var pageCategory= this._page||this._categoryName;
      switch(pageCategory){
        case "home":
          pageCategory = "homepage"
          break
        case "list":
          pageCategory = "products selector page"
          break
        case "detail":
      ...
      if(!window["appLoaded"]){
        ...
        json.virtualPageURL = window.location.pathname + window.location.search + window.location.hash;
        json.virtualPageTitle = document.title;
      }
      dataLayer.push(json)
    }
    if ('_page' in changed) {
